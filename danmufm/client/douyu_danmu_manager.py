...
class DouyuDanmuManager(object):
    ...
    def __init__(self, auth_dst_ip, auth_dst_port,room,configp):
        config=configp
        print(config)
        ...
        if os.path.exists("./"+self.room_id)==False:
            os.makedirs('./'+self.room_id)
        self.filenamegift="./"+self.room_id+"/"+self.timestamp()+'gift.json'

        self.filename="./"+self.room_id+"/"+self.timestamp()+'danmu.json'
    def start(self):
        self.do_login()
    ...
    def storegift(self,data):
        
        with open(self.filenamegift, 'a') as json_file:
            json_file.write(json.dumps(data)+"\n")
