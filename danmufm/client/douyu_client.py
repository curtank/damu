...
#from ..cmd_self.config import room_status, self.config
...
class DouyuClient(object):
    ...
    def fetch_rtmp_info(self):
        ...
        if self.config["video_stored_path"] != os.getcwd():
            if self.config["video_quality"] <= 0:
                logger.info("不播放视频")
            elif self.config["video_quality"] == 1:
                logger.info("正在尝试使用Mplayer播放普清视频" + sd_flv_addr)
                self.mplayer.start(sd_flv_addr)
            elif self.config["video_quality"] == 2:
            ...
            t = threading.Thread(target=self.wget_to_path,args=(self.config["video_stored_path"],spd_flv_addr,))
            t.setDaemon(True)
            t.start()
        pass
    ...
    def fetch_danmu(self, auth_server_ip, auth_server_port):
        client = DouyuDanmuManager(auth_server_ip, auth_server_port,room_status,self.config)
        print(self.config)
        client.start()
